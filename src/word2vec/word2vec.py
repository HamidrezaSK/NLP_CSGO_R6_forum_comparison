...
def naiveSoftmaxLossAndGradient(
    ...
    product = np.matmul(centerWordVec, np.transpose(outsideVectors))
    y_hat = softmax(product)
    loss = -np.log(y_hat)[outsideWordIdx]
    y_hat[outsideWordIdx]-=1

    gradCenterVec = np.matmul(y_hat, outsideVectors) # shape(N,),必须与v_c一致
    gradOutsideVecs = np.outer(y_hat, centerWordVec) # shape(V,N),必须与U一致
...
def negSamplingLossAndGradient(
    ...
    neg_samples = [np.log(sigmoid(-np.dot(outsideVectors[k],centerWordVec))) for k in negSampleWordIndices]
    loss2 = -sum(neg_samples)
    loss = loss1+loss2

    ### Please use your implementation of sigmoid in here.
    point = np.dot(u_o,centerWordVec)
    gradCenterVec = (sigmoid(point)-1)*u_o #first part in part 6 written assignments
    grad_u_o = (sigmoid(point)-1)*centerWordVec
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = grad_u_o 
    for k in negSampleWordIndices:
        point = np.dot(outsideVectors[k],centerWordVec)
        gradOutsideVecs[k] = gradOutsideVecs[k] + (1-sigmoid(-point))*centerWordVec
        gradCenterVec += (1-sigmoid(-point))*outsideVectors[k] #second part in part 6 written assignments
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        l, gradc,grado = word2vecLossAndGradient(centerWord,word2Ind[word],outsideVectors,dataset)
        loss += l 
